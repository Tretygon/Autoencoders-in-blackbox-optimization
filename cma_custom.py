...
import numba
spec = [
    ('_D', numba.float32[:]),               # a simple scalar field
    ('_p_sigma', numba.float32[:]),               # a simple scalar field
...
class CMA:
    ...
    _sigma:float
    # _D: Optional[np.ndarray]
    # _B: Optional[np.ndarray]

    # _bounds:np.ndarray 
    _n_max_resampling:int
    ...
    def __init__(
        ...
        self._popsize:numba.int32 = population_size
        self._mu:float = mu
        ...
        self._c1:float = c1
        self._cmu:float = cmu
        self._c_sigma:float = c_sigma
        self._d_sigma:float = d_sigma
        self._cm:float = cm

        # E||N(0, I)|| (p.28)
        self._chi_n:float = math.sqrt(self._n_dim) * (
